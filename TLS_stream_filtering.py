...
malicious_path = input('Введите путь до файла с зловредным трафиком:')
benign_files_directory = os.listdir(benign_path)
malicious_files_directory = os.listdir(malicious_path)

# Создаем словари для 1 миллиона самых посещаемых сайтов в мире, наборов шифров, поддерживаемых групп и расширений
benign_urls = {}
...
# Создаем списки, в которых каждый поток tls хранится как объект словаря
malicious_jsons = []
benign_jsons = []
list_of_features = ["Src_Port", "Dst_Port", "Bytes_in", "Bytes_out", "Pkts_in", "Pkts_out", "entropy", "byte_dist_std", "byte_dist_mn", "num_of_exts"]

# Создаем словарь полезных URL-адресов из csv-файла Magestic Millions
for i in benign_url_file.index:
    benign_urls[str(benign_url_file["Domain"][i])] = ""

# Получение функций в словаре
def get_features(json_obj):
...
# Функция, которая распаковывает и декодирует имя сервера
def unpad_decode(hex):
    server_name_hex = hex
    server_name = bytes.fromhex(server_name_hex[10:]).decode("ascii").split('.')
    return server_name[len(server_name)-2] + "." + server_name[len(server_name)-1]

# Функция, возвращающая двоичный вектор поддерживаемых групп
def get_sg_and_ec_points_vector(list):
    supported_groups_vector = [0] * len(supported_groups_dict)
    ec_points_vector = [0] * len(ec_point_formats_dict) 
    for ext in list:
        if 'supported_groups' in ext:
            for i, key in enumerate(supported_groups_dict.keys()):
                if key == ext['supported_groups']:
                    supported_groups_vector[i] = 1             
        if 'ec_point_formats' in ext:
            for i, key in enumerate(ec_point_formats_dict.keys()):
                if key == ext['ec_point_formats']:
                    ec_points_vector[i] = 1
    return([supported_groups_vector, ec_points_vector])                

# Функция, возвращающая двоичный вектор наборов шифров
def get_cipher_vector(list):
    cipher_suite_vector = [0] * len(cipher_suites_dict)  
    for i, key in enumerate(cipher_suites_dict.keys()):
        if key in list:
            cipher_suite_vector[i] = 1          
    return cipher_suite_vector

# Функция, возвращающая двоичный вектор расширений
def get_extensions_vector(list):
    extensions_vector = [0] * len(extensions_dict)
    num_of_extensions = 0   
    for i, key in enumerate(extensions_dict.keys()):
        for ext in list:
            if key == ext:
                num_of_extensions += 1
                extensions_vector[i] = 1         
    return [num_of_extensions, extensions_vector]

# Создаем объект json из уменьшенного набора данных
def clean_json(json_obj):
    # Получаем все бинарные векторы из словаря
    ...
    cs_binary_vector = get_cipher_vector(json_obj['tls']['cs'])
    sg_binary_vector, ec_binary_vector = get_sg_and_ec_points_vector(json_obj['tls']['c_extensions'])
    num_of_extensions, ext_binary_vector = get_extensions_vector(json_obj['tls']['c_extensions'])

    # Добавление функций в список функций для отправки в CSV-файл
...
# Функция для удаления пакетов "хорошего" трафика
def server_check(json_obj):
    count = 0
    try:
        server_name = ""
        for ext in json_obj['tls']['c_extensions']:
            if 'server_name' in ext:
                server_name = unpad_decode(ext['server_name']) # декодируем server_name из hex в ascii
            if len(malicious_jsons) >= 0: 
                for name, value in benign_urls.items(): # берем значения из csv-файла с доверенными ip адресами
                    count+=1
                    if name == server_name: # если доверенный, то не записываем его в malicious csv 
                        break
                    if name != server_name and count == 1000002:
                        # если da ip не совпадает ни с одним из доверенных ip (их всего 1000002), то добавляем его в malicious csv 
                        malicious_jsons.append(json_obj)
                        get_features(json_obj)
    except:
        malicious_jsons.append(json_obj)
        get_features(json_obj)
        
# Функция, которая отфильтровывает "хорошие" или вредоносные json-файлы, не имеющие обязательных функций
def filter_files(mal_or_ben, path_name, directory):
                        ...
                        and tls_dict['num_pkts_in'] and tls_dict['num_pkts_out']:
                            ...
                            for ext in pkt['c_extensions']:
                                ...
                                if 'ec_point_formats' in ext:
                                    temp+=1
                            if temp==2:
                                if (mal_or_ben == "ben"): # Весь хороший трафик записываем сразу
                                    get_features(tls_dict)
                                    benign_jsons.append(tls_dict)
                                elif(mal_or_ben == "mal"): # Для плохого трафика сначала запускаем предобработку serverCheck
                                    server_check(tls_dict)
                    else:
                        break
            finally:
                joy_json_output.close()

# Функция для получения метаданных функций (cs_bv -> ext_bv -> sg_bv -> ec_bv)
def finalize_list_of_features():
...
# Функция для добавления извлеченных функций в csv
def features_to_CSV(mal_or_ben):
    file_name = ""
    if mal_or_ben == "ben":
        file_name = "benign_flows_.csv"
    elif mal_or_ben == "mal":
        file_name = "malicious_flows_.csv"
...
# Вызов функций для начала процесса очистки и извлечения признаков
filter_files("ben", benign_path, benign_files_directory)
filter_files("mal", malicious_path, malicious_files_directory)

# Выводим количество получившихся потоков
print("Malicious Flows: ", len(malicious_jsons), " Benign Flows: ", len(benign_jsons))

# Вызов функций для записи потоков tls в файлы csv
finalize_list_of_features()
features_to_CSV("ben")#, benign_jsons)
features_to_CSV("mal")#, malicious_jsons)
